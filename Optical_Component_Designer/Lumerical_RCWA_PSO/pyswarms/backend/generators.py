...
def generate_swarm(
    ...
    try:
        if (init_pos is not None) and (bounds is None):
            ...
            lb, ub = bounds
            # Same contraints for all the particles 
            ...
            period_vect = (pos[:,-1]/2) - 50e-9
            greater_than_bound_loc = np.nonzero(pos[:,0:-4:2] > period_vect[:, np.newaxis])
            ...
            for row_idx in np.unique(greater_than_bound_loc[0]):
                ...
                max_r = (pos[row_idx, -1]/2) - 50e-9 # 50 nm  buffer from the edge of the FDTD region
                max_bounds[row_idx, 0:-4:2] = max_r  
                new_pos = np.squeeze(center * np.random.uniform(low=min_bounds[0], high=max_bounds[row_idx], size=(1, dimensions)))
                # Add new vertices to the previous generated period and layer thickness
                new_pos = np.concatenate((new_pos[:-1], np.array([pos[row_idx, -1]]))) 
                pos[row_idx] = new_pos
...
def create_swarm(
    ...
    # PSO FOM maximization or minimization
    if minimize_fom:
        best_cost_ini = np.inf
    else:
        best_cost_ini = -np.inf

    return Swarm(position, velocity, options=options, best_cost=best_cost_ini)
