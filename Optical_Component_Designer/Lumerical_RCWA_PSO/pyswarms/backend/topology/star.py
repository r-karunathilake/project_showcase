...
class Star(Topology):
    ...
    def compute_gbest(self, swarm, minimize: bool):
        ...
        try:
            ...
            if not minimize and np.max(swarm.pbest_cost) > swarm.best_cost:
                # Get the particle position with the highest pbest_cost
                # and assign it to be the best_pos
                best_pos = swarm.pbest_pos[np.argmax(swarm.pbest_cost)]
                best_cost = np.max(swarm.pbest_cost)
                
            elif minimize and np.min(swarm.pbest_cost) < swarm.best_cost:
