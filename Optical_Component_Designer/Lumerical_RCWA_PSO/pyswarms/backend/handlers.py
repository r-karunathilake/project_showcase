...
class BoundaryHandler(HandlerMixin):
    ...
    def bounded_struct(self, position, bounds, **kwargs) -> np.ndarray:
        """This function will ensure the generated shapes are always
        bounded within the generated period. Hence, this is a method for
        handling bounded parameter constraints of vertex radius and period.
        Args:
            position: particles and their positions to be checked 
            bounds: original bounds of the generated particles
        
        Returns:
            Properly bounded random particle positions for the generation 
        """
        #print(f"Position before the boundary handler: {position}")
        lb, ub = bounds

        # Call shrink handler
        position = self.shrink(position, bounds)

        # Fix radius parameters that are larger than period/2 - 50 nm buffer of the FDTD region (last parameter in 'lb' and 'ub')
        period_vect = (position[:, -1]/2) - 50e-9
        greater_than_bound_loc = np.nonzero(position[:,0:-4:2] > period_vect[:, np.newaxis])

        # Fix all the particles radius parameters to a random position inside the period
        for row_idx in np.unique(greater_than_bound_loc[0]):
            print(f"Particle Number #{row_idx+1}")
            print("\tBefore fix:")
            print(f"\t\tPeriod (nm): {position[row_idx, -1]/1e-9:.2f}")
            print(f"\t\tMax radius (nm): {np.max(position[row_idx, 0:-4:2])/1e-9:.2f}")
            print(f"\t\tPCM Top (nm): {position[row_idx, -4]/1e-9:.2f}")
            print(f"\t\tPCM Bottom (nm): {position[row_idx, -3]/1e-9:.2f}")
            print(f"\t\tTotal Height (nm): {position[row_idx, -2]/1e-9:.2f}")

            # Each particle has a different period, and hence different max radius 
            max_r = (position[row_idx, -1]/2) - 50e-9 # 50 nm  buffer from the edge of the FDTD region
            ub[0:-4:2] = max_r  
            pos = np.squeeze(1.00 * np.random.uniform(low=lb, high=ub, size=(1, position.shape[1])))
            # Add new vertices and layer thicknesses to the previous generated period
            pos = np.concatenate((pos[:-1], np.array([position[row_idx,-1]]))) 
            position[row_idx] = pos
        ...
        #print(f"Position after the boundary handler: {position}")
        return position 
    ...
    def shrink(self, position, bounds, **kwargs):
        ...
        if self.memory is None:
            ...
            sigma[lower_than_bound] = np.abs((lb[lower_than_bound[1]] - self.memory[lower_than_bound]) / velocity[lower_than_bound])
            sigma[greater_than_bound] = np.abs((ub[greater_than_bound[1]] - self.memory[greater_than_bound]) / velocity[greater_than_bound])
            min_sigma = np.amin(sigma, axis=1)

            new_pos = position
            new_pos[lower_than_bound[0]] = (self.memory[lower_than_bound[0]] + np.multiply(min_sigma[lower_than_bound[0]], velocity[lower_than_bound[0]].T,).T)
            new_pos[greater_than_bound[0]] = (self.memory[greater_than_bound[0]] + np.multiply(min_sigma[greater_than_bound[0]], velocity[greater_than_bound[0]].T,).T)

            # Dividing by zero error due to previous boundary handler call and update to radius values 
            if np.isinf(min_sigma).any():
                error_particles = np.nonzero(np.isnan(new_pos))
                # The previous best particle keeps its position
                idx = np.unique(error_particles[0])
                new_pos[idx] = self.memory[idx]
    ...
    def random(self, position, bounds, **kwargs):
        ...
        # Index 0 in 'greater_than_bound' presents the row index of out of bound particles
        for bad_particle_g in np.unique(greater_than_bound[0]):
            # Move the whole particle to a random location in the bounded solution space 
            new_pos[bad_particle_g] = np.array([np.array([u - l for u, l in zip(ub, lb)]) * np.random.random_sample((position.shape[1],)) + lb])

        # Index 0 in 'lower_than_bound' presents the row index of out of bound particles
        for bad_particle_l in np.unique(lower_than_bound[0]):
            # This particle position should have been already fixed. Nothing to do          
            if bad_particle_l in greater_than_bound[0]:
                continue

            new_pos[bad_particle_l] = np.array([np.array([u - l for u, l in zip(ub, lb)]) * np.random.random_sample((position.shape[1],)) + lb])
    ...
    def periodic(self, position, bounds, **kwargs):
        ...
        This method resets the particles that exceed the bounds by using the
        ...
        if lower_than_bound[0].size != 0 and lower_than_bound[1].size != 0:
            new_pos[lower_than_bound] = ub[lower_than_bound] - np.mod((lb[lower_than_bound] - new_pos[lower_than_bound]), bound_d[lower_than_bound])
        
        if greater_than_bound[0].size != 0 and greater_than_bound[1].size != 0:
            new_pos[greater_than_bound] = lb[greater_than_bound] + np.mod((new_pos[greater_than_bound] - ub[greater_than_bound]), bound_d[greater_than_bound])
...
class VelocityHandler(HandlerMixin):
    ...
    def adjust(self, velocity, clamp=None, **kwargs):
        ...
        """
        new_position = kwargs["position"] + velocity
        try:
            if self.memory is None:
                new_vel = velocity
                self.memory = kwargs["position"]
            else:
                new_vel = new_position - self.memory
    ...
    def zero(self, velocity, clamp=None, **kwargs):
        ...
        try:
            # Calculate the expected new position by adding new velocity to current position 
            new_position = kwargs["position"].copy() + velocity

            # Check which swarm particles are outside the defined bounds due to new velocity 
            lower_than_bound, greater_than_bound = self._out_of_bounds(new_position, kwargs["bounds"])

            # Update the new velocity to all zeros for particles that would be out of bounds 
            velocity[np.unique(lower_than_bound[0])] = np.zeros(velocity.shape[1])
            velocity[np.unique(greater_than_bound[0])] = np.zeros(velocity.shape[1])
        except KeyError:
            self.rep.logger.exception("Keyword 'bounds' missing")
            raise
        else:
            return velocity
