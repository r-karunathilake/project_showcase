...
def compute_pbest(swarm, minimize: bool):
...
def compute_velocity(swarm, clamp, vh, bounds=None):
    ...
    try:
        ...
        rep.log(f"Options for velocity calculation: c1:{c1}, c2:{c2}, w:{w}", lvl=10)
...
def compute_objective_function(swarm, objective_func, pool=None, **kwargs):
    ...
    """
    if pool is None:
        rep.log(f"Swarm positions: {swarm.position}", lvl=10)
        return objective_func(swarm.position, **kwargs)
    else:
        results = pool.map(partial(objective_func, **kwargs), np.array_split(swarm.position, pool._id))
