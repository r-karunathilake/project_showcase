...
public class Knight extends Piece{
    ...
    // For a knight, these are all the coordinate offsets for a
    // knight position with the largest degree of freedom (for a 8x8 tile board).
        ...
        for(final int currentCandidateOffset:CANDIDATE_MOVE_OFFSETS){
            ...
            if (BoardUtils.isValidTileCoordinate(candidateDestinationCoordinate)){
                ...
                if(isFirstColumnExclusion(this.piecePosition, currentCandidateOffset)   ||
                   ifSecondColumnExclusion(this.piecePosition, currentCandidateOffset)  ||
                   ifSeventhColumnExclusion(this.piecePosition, currentCandidateOffset) ||
                   ifEighthColumnExclusion(this.piecePosition, currentCandidateOffset)){
    ...
    private static boolean ifSecondColumnExclusion(final int currentPos, final int candidateOffset){
        return BoardUtils.SECOND_COLUMN[currentPos] && (candidateOffset == -10 || candidateOffset == 6);
    }

    private static boolean ifSeventhColumnExclusion(final int currentPos, final int candidateOffset){
        return BoardUtils.SEVENTH_COLUMN[currentPos] && (candidateOffset == -6 || candidateOffset == 10);
    }

    private static boolean ifEighthColumnExclusion(final int currentPos, final int candidateOffset){
